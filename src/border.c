...
void border_draw(const struct Border *b)
{
    ...
    dest_rect.y = 0;
    SDL_RenderTexture(b->renderer, b->image, &b->src_rects[b->theme], &dest_rect);

    dest_rect.x = b->piece_width * (float)(b->columns + 1) - b->left_offset;
    dest_rect.y = 0;
    SDL_RenderTexture(b->renderer, b->image, &b->src_rects[2 + b->theme], &dest_rect);

    dest_rect.x = -b->left_offset;
    dest_rect.y = b->piece_width * (float)b->rows + b->piece_height;
    SDL_RenderTexture(b->renderer, b->image, &b->src_rects[5 + b->theme], &dest_rect);

    dest_rect.x = b->piece_width * (float)(b->columns + 1) - b->left_offset;
    dest_rect.y = b->piece_width * (float)b->rows + b->piece_height;
    SDL_RenderTexture(b->renderer, b->image, &b->src_rects[7 + b->theme], &dest_rect);

    for (unsigned column = 0; column < b->columns; column++)
    {
        dest_rect.x = b->piece_width * (float)(column + 1) - b->left_offset;
        dest_rect.y = 0;
        SDL_RenderTexture(b->renderer, b->image, &b->src_rects[1 + b->theme], &dest_rect);

        dest_rect.y = b->piece_width * (float)b->rows + b->piece_height;
        SDL_RenderTexture(b->renderer, b->image, &b->src_rects[6 + b->theme], &dest_rect);
    }
    ...
    for (unsigned row = 0; row < b->rows; row++)
    {
        ...
        SDL_RenderTexture(b->renderer, b->image, &b->src_rects[3 + b->theme], &dest_rect);

        dest_rect.x = b->piece_width * (float)(b->columns + 1) - b->left_offset;
        SDL_RenderTexture(b->renderer, b->image, &b->src_rects[4 + b->theme], &dest_rect);
    }
}
