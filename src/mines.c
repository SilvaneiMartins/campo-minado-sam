...
bool mines_new(struct Mines **mines, SDL_Renderer *renderer)
{
    *mines = calloc(1, sizeof(struct Mines));
    ...
    m->back_dest_rects.x = DIGIT_BACK_LEFT * 2;
    m->back_dest_rects.y = DIGIT_BACK_TOP * 2;
    m->back_dest_rects.w = DIGIT_BACK_WIDTH * 2;
    m->back_dest_rects.h = DIGIT_BACK_HEIGHT * 2;
